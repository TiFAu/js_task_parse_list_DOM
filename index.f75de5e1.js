...
].find((h)=>h.textContent === "List of employees");
let targetList = targetHeading?.nextElementSibling;
while(targetList && targetList.tagName !== "UL")targetList = targetList.nextElementSibling;
 ...
 */ function sortList(list) {
    list.sort((person1, person2)=>convertStringToNumber(person2.dataset.salary) - convertStringToNumber(person1.dataset.salary)).forEach((person)=>list[0]?.closest("ul").append(person));
}
/**
 * @param {HTMLLIElement[]} list
 * @returns {Object[]}
